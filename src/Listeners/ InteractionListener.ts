...
export default class InteractionCreate extends Event {
  ...
  async run(i: Interaction) {
    if (i.isCommand()) {
      const cmd =
        this.client.commands.get(i.commandName) ||
        this.client.commands.get(this.client.aliases.get(i.commandName));
      if (cmd) {
        if (cmd.commandSettings.admin || cmd.commandSettings.roles[0]) {
          let member = i.guild.members.cache.get(i.user.id);
          if (
            !member.permissions.has("ADMINISTRATOR") &&
            !cmd.commandSettings.roles.some((x) => member.roles.cache.has(x))
          )
            return i.reply({
