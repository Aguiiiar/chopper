...
export default class CommandLoader {
  constructor(client: Chopper) {
    readdir(
      `./${process.env.ENV == "PROD" ? "dist" : "src"}/Commands`,
      (err, f) => {
        if (err) throw new RangeError("Command Loader Error: " + err);
        f.forEach((category) => {
          readdir(
            `./${
              process.env.ENV == "PROD" ? "dist" : "src"
            }/Commands/${category}`,
            async (err, cmd) => {
              for (let i = 0; i < cmd.length; i++) {
                ...
                delete require.cache[
                  require.resolve(`../Commands/${category}/${e}`)
                ];
                ...
                command.commandSettings.aliases.forEach((aliases) =>
                  client.aliases.set(aliases, command.commandSettings.name)
                );
